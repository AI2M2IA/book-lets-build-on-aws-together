...
**Özet: Depolama Bitti, Sırada Veritabanları Var**
...
Karşılaştırın: 1 yıllık RI ile sabit bir db.r6g.2xlarge (p95 yükünü kaldıracak şekilde boyutlandırılmış, tahmini sağlanmış eşdeğerleri): 0,48 dolar/saat × 0,60 (RI indirimi) × 730 = 210 dolar/ay.

“RI daha ucuz,” dedi Leo.

“Sabit bir yük için, evet,” dedi Tom. “Ama dağılıma bak. Düşük trafik dönemimiz — Pazartesi’den Perşembe’ye, 02.00–07.00 — ortalama 0,8 ACU. Sabit bir sağlanmış instance’ta, o saatlerde kullandığımızın 8 katı için ödeme yapardık, sadece boşta dururken.”
...
Sağlanmış seçenek: 1 yıllık Rezervasyonlu Instance ile bir db.r6g.2xlarge. Maliyet: 0,48 dolar/saat On-Demand × 0,60 (RI indirimi) × 730 saat = 210 dolar/ay. Yükten bağımsız, sabit.
...
Sağlanmış bir RI’da: 210 dolar/ay. Serverless: 368 dolar/ay. Sağlanmış seçenek ayda 158 dolar tasarruf ediyordu.

“Bu apaçık görünüyor,” dedi Leo. “Neden Serverless’ta kalıyoruz?”

“Çünkü 368 dolar ortalama,” dedi Tom. “Cuma akşamlarına bak.”

Cuma 18.00–22.00: ortalama 14,1 ACU. O dört saatlik pencere için, Serverless saatte 1,692 dolara mal oluyor. Ayda 210 dolardan sağlanmış bir db.r6g.2xlarge — maksimum kapasitesi — 8 vCPU’ydu. Serverless kümesi o pencere boyunca kabaca 16 vCPU eşdeğeri çalıştırıyordu.

“Cuma zirvemiz için boyutlandırılmış bir sağlanmış instance bir db.r6g.4xlarge olurdu,” dedi Tom. “RI oranında, bu 0,96 dolar/saat × 0,60 = 0,576 dolar/saat. Aylık: 420 dolar/ay.”

“Bu, 368 dolarlık Serverless ortalamasından fazla,” dedi Maya.

“Doğru. Ve sağlanmış instance’ı hafta içi temeline göre boyutlandırsaydık — db.r6g.2xlarge — Cuma geceleri bir sorun olurdu. Zirve yükünde, 8 vCPU’luk bir instance’ta 14 ACU eşdeğerine ulaşırdık. Bu, CPU doygunluğu.”
...
| Sağlanmış RI (r6g.2xl) | 210 $ | 210 $/730saat = 0,288 $/saat | sınırlı — doygunluk riski |
| Sağlanmış RI (r6g.4xl) | 420 $ | 0,576 $/saat | rahat boşluk |

“Serverless seçeneği 368 dolar,” dedi Tom. “Doğru boyutlandırılmış sağlanmış seçenek 420 dolar — ve bu, önümüzdeki çeyrekte trafik örüntülerimiz değiştiğinde sağlanmış instance’ı izleme ve manuel ölçeklendirmenin operasyonel maliyetini hesaba katmadan önce.”
...
Instance fazla sağlanmıştı. Bir cache.r6g.medium muhtemelen yükü boşlukla kaldırırdı.
...
Medium instance’ta, maxMemoryPolicy `allkeys-lru` olarak ayarlanmıştı — bellek dolduğunda, Redis yer açmak için en az kullanılan anahtarı çıkarırdı. Bu, genel bir önbellek için doğru politikadır. Ama mutabakat toplu işi için, önbellekteki her anahtara aktif olarak ihtiyaç vardı. Bellek, medium instance’ın 6,38 GB’ının %84’ünde dolduğunda, Redis anahtarları çıkarmaya başladı. Her çıkarma bir önbellek ıskasıydı. Her önbellek ıskası, çıkarılan değeri ham işlem kayıtlarından yeniden hesaplamak için altta yatan PostgreSQL veritabanına bir sorgu gönderdi.
...
Nimbus ElastiCache kümesinin kendi yüksek riskli işlemi vardı: Cuma akşam yemeği telaşı. Tom o veriye sahipti — üç ardışık Cuma gecesi, r6g.large’da %58 bellek kullanımına ulaşmıştı. r6g.medium’a geçer ve sipariş işleme boru hattındaki bir şey daha fazla önbellek alanı kullanmak için değişirse — yeni bir özellik, farklı bir önbellekleme stratejisi — o %58, %80 olabilirdi ve medium’da %80, çıkarma bölgesiydi.

Yine de sayıları hesapladı. r6g.large’dan r6g.medium’a geçmek: ayda 730 saat çalışan, 0,127 dolar/saatten iki düğüme karşı 0,065 dolar/saatten iki düğüm. Large: 185 dolar/ay. Medium: 95 dolar/ay. Potansiyel tasarruf: 90 dolar/ay. Medium instance’ı staging’de iki hafta yük altında test etti. Bellek %71’de zirve yaptı — limite, rahatsız olacak kadar yakın.
...
“Aynı instance boyutunda Rezervasyonlu Düğümlerde tasarruf edeceğim 65 dolar/ay gerçek bir tasarruf,” dedi Tom. “Medium’a geçerek tasarruf edeceğim 90 dolar/ay, Cuma akşam yemeği telaşını riske atıyorsa sahte bir ekonomidir. Bazen daha küçük bir instance’a doğru boyutlandırma bir performans olayını riske atar — Rezervasyonlu Düğümler tasarrufun çoğunu hiçbir riske girmeden verir.”

r6g.large için Rezervasyonlu Düğümleri satın aldı.

“Aylık tasarruftaki 25 dolarlık fark,” dedi Tom, “bir Cuma gecesi olayına değmez.”
